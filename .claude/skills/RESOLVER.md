...
6. **`tac-*` beats general external plugins.** If a third-party skill plugin is installed (e.g., Superpowers `obra/superpowers`, which ships generic brainstorming / TDD / debugging / code-review / skill-authoring skills), the matching **`tac-*` skill is authoritative for any tac-express task and always wins** — it carries the domain model, the Fourteen Laws, and the enforced gates (`audit:skills`, routing evals) that a generic skill lacks. Equivalences where `tac-*` wins: brainstorming → `tac-brainstorming`, TDD → `tac-tdd`, debugging → `tac-debug`, code review → `tac-code-review`, skill authoring → `tac-skillify`. Use a general external skill **only** for an intent no `tac-*` skill covers, or to *borrow a pattern* (e.g., Superpowers' subagent-driven-development + built-in-review loop → apply it through [`conventions/subagent-routing.md`](conventions/subagent-routing.md)). Never let an external plugin override the mandatory `tac-*` dispatch from `CLAUDE.md §0.5`.
