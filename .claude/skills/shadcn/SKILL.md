...
> **IMPORTANT:** This project is **pnpm-only** (Fourteen Laws). Run every shadcn CLI command with `pnpm dlx shadcn@latest` — and pin `shadcn@4.7.0` to match `.mcp.json` and the `@tac` registry. Never use `npm`, `yarn`, or other runners. Project-specific rules live in [`tac-shadcn`](../tac-shadcn/SKILL.md).
