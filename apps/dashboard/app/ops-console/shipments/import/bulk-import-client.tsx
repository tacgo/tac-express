  ...
  async function handleSubmit() {
    ...
    const payload = validRows.map((r) => ({
      ...
      chargeable_weight: Number(
        r.data.chargeable_weight ?? r.data.dead_weight ?? 0
      ),
    ...
    <PageShell width="wide">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Stat
          ...
          tone={invalidRows.length > 0 ? "danger" : "ok"}
        />
      </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {validRows.length === 1 ? "shipment" : "shipments"}
              </Button>
            </div>
          </div>

          <div className="border border-border">
            <div className="grid grid-cols-[60px_120px_1fr] border-b border-border bg-muted/40 px-3 py-2 font-mono text-2xs tracking-wider text-muted-foreground uppercase">
              <span>Row</span>
              <span>Status</span>
              <span>Detail</span>
            </div>
              ...
              >
                ...
                <span>
                  {r.errors.length === 0 ? (
                    <Badge
                      variant="default"
                      className="border-accent-success bg-accent-success/15 text-accent-success-foreground"
                    >
          ...
          {submitted && (
            <div className="tac-fui-panel border-l-4 border-l-accent-success bg-card p-4">
              <p className="font-mono text-xs tracking-wider text-accent-success uppercase">
                Import complete
              </p>
              <p className="mt-1 text-sm text-foreground">
                Created <span className="font-mono">{submitted.inserted}</span>{" "}
                shipments.{" "}
                {submitted.failed > 0 && (
                  <span className="text-muted-foreground">
                    Skipped{" "}
                    <span className="font-mono">{submitted.failed}</span> rows
                    with errors.
                  </span>
                )}
              </p>
...
function parseCsv(text: string): ParsedRow[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (data.pieces && Number.isNaN(Number(data.pieces)))
      errors.push("pieces is not a number")
    if (data.dead_weight && Number.isNaN(Number(data.dead_weight)))
      errors.push("dead_weight is not a number")
