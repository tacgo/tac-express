...
export function OpsDashboardLive({ initialKpis }: OpsDashboardLiveProps) {
  ...
  // ⚡ Bolt: Memoize mapped API data to prevent breaking referential
  // equality and triggering unnecessary table re-renders on every render.
  const upcomingItems = React.useMemo(
    () =>
      (upcomingQuery.data ?? []).map((op: UpcomingOp) => ({
        id: op.id,
        label: op.title,
        eta: op.eta,
        etaDate: op.etaDate,
