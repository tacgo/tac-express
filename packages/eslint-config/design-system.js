...
/**
 * Selectors for the monorepo-wide `no-restricted-syntax` rule.
 * Exported so per-package scoped configs can extend them without losing coverage.
 */
export const designSystemSyntaxSelectors = [
...
/**
 * Selectors forbidding raw interactive HTML elements in composed components.
 * Applied only to `packages/ui/src/components/composed/**` and
 * `apps/dashboard/app/**` via per-package scoped ESLint config blocks.
 *
 * Permitted exceptions (document with eslint-disable-next-line + reason):
 *   • Radix asChild wrappers — PopoverTrigger/DropdownMenuTrigger/etc. asChild
 *     requires a native element; the primitive IS the child.
 *   • Print views — invoice-print-view.tsx, manifest-print-view.tsx use <table>
 *     for print layout where CSS grid is not print-safe.
 *   • data-table.tsx — canonical DataTable primitive with native <table> for
 *     proper ARIA grid semantics + filter <input>.
 *   • <input type="file"> inside <label> — native file picker pattern.
 */
export const rawInteractiveSelectors = [
  {
    selector: "JSXOpeningElement[name.name='button']",
    message:
      "❌ [TAC] Raw <button> in composed/dashboard component. Use <Button> from @workspace/ui/components/button. Exception: add eslint-disable-next-line comment for Radix asChild wrappers.",
  },
  {
    selector: "JSXOpeningElement[name.name='input']",
    message:
      "❌ [TAC] Raw <input> in composed/dashboard component. Use <Input> from @workspace/ui/components/input. Exception: add eslint-disable-next-line for cmdk/Radix asChild or <input type='file'> inside <label>.",
  },
  {
    selector: "JSXOpeningElement[name.name='select']",
    message:
      "❌ [TAC] Raw <select> in composed/dashboard component. Use <Select> from @workspace/ui/components/select. Exception: add eslint-disable-next-line for native selects bound directly to RHF register() or simple onChange without Controller.",
  },
  {
    selector: "JSXOpeningElement[name.name='textarea']",
    message:
      "❌ [TAC] Raw <textarea> in composed/dashboard component. Use <Textarea> from @workspace/ui/components/textarea.",
  },
  {
    selector: "JSXOpeningElement[name.name='table']",
    message:
      "❌ [TAC] Raw <table> in composed/dashboard component. Use DataTable from @workspace/ui/components/data-table, or add eslint-disable-next-line for print-view and display-only tables.",
  },
]
...
export const designSystemConfig = {
  rules: {
     ...
     */
    "no-restricted-syntax": ["error", ...designSystemSyntaxSelectors],
  },
}
